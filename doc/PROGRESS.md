...
### 问题描述
经过测试，发现第一次修复不完整。问题依然存在。

### 问题根因
经过深入分析，发现真正的问题在于 `updateSongInfo()` 函数的设计：
...
这段代码只在 `titleText` 存在 **且** 与当前显示不同时才更新。

但云端播放器的 DOM 元素 `.myhksong span` 可能：
1. 根本不存在（DOM 还没加载完成）
2. 存在但内容为空（播放器还没获取到歌曲）
3. 存在且有内容，但之前已经被设置为"等待音频加载..."

这意味着如果之前歌名被设置为"等待音频加载..."，而云端播放器 DOM 没有正确同步，歌名将永远停留在"等待音频加载..."。

### 解决方案

**1. 优化 `updateSongInfo()` 函数**（第4034-4065行）
- 移除条件检查 `titleText && el.title.innerText !== titleText`
- 简化为只要有 `titleText` 就更新
- 移除不必要的 `resized` 变量，改为检查 `titleText || artistText`

**2. 优化 `emptied` 事件处理**（第3211-3221行）
- 首先调用 `updateSongInfo()` 从云端获取歌名
- 只有在 `readyState < 2`（音频未加载）时才显示等待状态
- 不再无条件设置歌名为"等待音频加载..."

**3. 简化 `onTimeUpdate()` 中的歌名更新**（第3290-3294行）
- 恢复为直接调用 `updateSongInfo()`
- 移除之前添加的条件检查

### 修改的文件
- `TuneFlow/TuneFlow.html`:
  - 优化 `updateSongInfo()` 函数逻辑
  - 简化 `emptied` 事件处理
  - 简化 `onTimeUpdate()` 中的歌名更新调用

## 2026-04-21 歌词交互优化

### 问题描述
歌词行的悬停和点击交互不够友好：
1. 需要点击左边的小播放按钮才能跳转，不符合用户习惯
2. 悬停时歌词清晰度不够

### 解决方案

**1. 优化歌词悬停效果**（第885-904行）
- 移除了歌词行左边的 `padding-left: 40px` 和播放按钮空间
- 添加悬停时的精美样式：
  - 圆角背景 `rgba(255, 255, 255, 0.08)`
  - 精致的 box-shadow 边框和内发光效果
  - 轻微放大交互反馈 `transform: scale(1.02)`
  - 文字颜色变亮 `color: rgba(255, 255, 255, 0.9)`
  - 原句和翻译透明度设为 1，移除模糊效果
- 使用 Material Design 标准缓动曲线

**2. 优化歌词点击跳转**（第3391-3430行）
- 现在点击歌词任意位置（包括文字本身）都能跳转
- 不再需要点击左边的小播放按钮
- 保留了播放按钮的点击功能作为备选
